...
def main():
    ...
    if gr.__version__ not in ['3.32.6']:
        raise ModuleNotFoundError("使用项目内置Gradio获取最优体验! 请运行 `pip install -r requirements.txt` 指令安装内置Gradio及其他依赖, "
                                  "详情信息见requirements.txt.")
    ...
    title_html = (f"<h1 align=\"center\">GPT Academic Special Edition</h1>"
                  f"<li align=\"center\">Release version {get_current_version()}</li>{theme_declaration}"
                  f"<h3 align=\"center\">This project has no relationship with King's College London, and is not an "
                  f"official Project of King's College London.</h3>")
    description = "This is a special edition of GPT Academic designed for King's students. "
    description += "</br></br>It is a writing and programming assistant based on GPT and llm-models. "
    description += "</br></br>Please respect the academic honesty policy of King's College London. "
    description += "</br></br>Please do not use this tool to cheat in exams or assignments. "
    description += "</br></br>For more information, please refer to the following links: "
    description += ("</br></br>[King's College London Student Guidance On Academic Honesty and Plagiarism]("
                    "https://www.kcl.ac.uk/campuslife/acservices/conduct/student-guidance-on-plagiarism.pdf)")
    description += ("</br></br>[King’s guidance on generative AI for teaching, assessment and feedback]("
    ...
    description += "</br></br>如何语音对话: 文档待更新"
    description += "</br></br>report Error, Bug or Suggestion: <br>Please contact the developer via email: "
    description += "<li>admin@tzpro.xyz</li>"
        ...
        with gr.Floating(init_x="0%", init_y="0%", visible=True, width=None, drag="forbidden"):
            with gr.Row():
                with gr.Tab("上传文件", elem_id="interact-panel"):
                    gr.Markdown(
                        "请上传本地文件/压缩包供“函数插件区”功能调用。请注意: 上传文件后会自动把输入区修改为相应路径。")
            ...
            with gr.Accordion("浮动输入区", open=True, elem_id="input-panel2"):
                with gr.Row() as row:
                    ...
                    with gr.Column(scale=10):
                        txt2 = gr.Textbox(show_label=False, placeholder="Input question here.", lines=8,
                                          label="输入区2").style(container=False)
            ...
            with gr.Accordion("自定义菜单", open=True, elem_id="edit-panel"):
                with gr.Row() as row:
                    with gr.Column(scale=10):
                        ...
                        basic_btn_dropdown = gr.Dropdown(AVAIL_BTN, value="自定义按钮1",
                                                         label="选择一个需要自定义基础功能区按钮").style(
                            container=False)
                    ...
                    with gr.Column(scale=1, min_width=70):
                        ...
                        h.then(None, [persistent_cookie], None,
                               _js="""(persistent_cookie)=>{setCookie("persistent_cookie", persistent_cookie, 5);}""")  # save persistent cookie
        ...
        def on_dropdown_changed(k):
            ...
            if plugins[k].get("AdvancedArgs", False):  # 是否唤起高级插件参数区
                ret.update({plugin_advanced_arg: gr.update(visible=True,
                                                           label=f"插件[{k}]的高级参数说明：" + plugins[k].get(
                                                               "ArgsReminder", [f"没有提供高级参数功能说明"]))})
            else:
